...
def func_zoomin() :
    ...
    photo2 = photo.copy()
    photo = photo2.resize((int(oriX*scale), int(oriY*scale)))
    newX = photo2.width
    newY = photo2.height
    displayImage(photo2, newX, newY)
...
def func_mirror1() :
    global window, canvas, paper, photo, photo2, oriX, oriY
    photo2 = photo.copy()
    photo = photo2.transpose(Image.FLIP_TOP_BOTTOM)
    newX = photo2.width
    newY = photo2.height
    displayImage(photo2, newX, newY)
...
def func_mirror2() :
    global window, canvas, paper, photo, photo2, oriX, oriY
    photo2 = photo.copy()
    photo = photo2.transpose(Image.FLIP_LEFT_RIGHT)
    newX = photo2.width
    newY = photo2.height
    displayImage(photo2, newX, newY)
...
def func_rotate():
    ...
    photo2 = photo.copy()
    photo = photo2.transpose(degree, expand = True)
    newX = photo2.width
    newY = photo2.height
    displayImage(photo2, newX, newY)
...
def func_bright() :
    global window, canvas, paper, photo, photo2, oriX, oriY
    degree = askfloat("밝게", "값을 입력하세요(1.0 ~ 10.0)", minvalue = 1.0, maxvalue = 10.0)
    photo2 = photo.copy()
...
def func_dark() :
    global window, canvas, paper, photo, photo2, oriX, oriY
    degree = askfloat("어둡게", "값을 입력하세요(0.0 ~ 1.0)", minvalue = 0.0, maxvalue = 1.0)
    photo2 = photo.copy()
    photo2 = imageEnhance.Brightness(photo2).enhance(value)
    newX = photo2.width
    newY = photo2.height
...
mainMenu.add_cascade(label = "확대", command = func_zoomin)
