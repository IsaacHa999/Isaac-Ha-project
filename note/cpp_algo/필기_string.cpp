...
### 생성 및 할당
- **`string()`**: 빈 문자열을 생성합니다.
- **`string(const string& str)`**: 복사 생성자, 다른 `std::string` 객체로부터 문자열을 복사합니다.
- **`string(const string& str, size_t pos, size_t len = npos)`**: 부분 문자열을 사용하여 문자열을 생성합니다.
- **`string(const char* s)`**: C 스타일 문자열로부터 `std::string` 객체를 생성합니다.
- **`string(size_t n, char c)`**: 문자 `c`를 `n`번 반복하여 문자열을 생성합니다.
- **`operator=`**: 문자열을 할당합니다.

### 크기 및 용량
- **`size()` 또는 `length()`**: 문자열의 길이(문자 수)를 반환합니다.
...
- **`push_back(char c)`**: 문자열의 끝에 문자를 추가합니다.
- **`pop_back()`**: 문자열의 마지막 문자를 제거합니다.

### 검색 및 비교
- **`find(const string& str, size_t pos = 0)`**: 주어진 위치에서부터 문자열을 검색하고, 발견된 첫 위치의 인덱스를 반환합니다.
- **`rfind(const string& str, size_t pos = npos)`**: 문자열을 역순으로 검색합니다.
- **`compare(const string& str)`**: 두 문자열을 비교하고 결과를 정수로 반환합니다.

### 부분 문자열 및 변환
- **`substr(size_t pos = 0, size_t len = npos)`**: 문자열의 부분을 추출하여 새 문자열을 생성합니다.
- **`c_str()`**: 문자열의 C 스타일 표현(널 종료 문자열)을 반환합니다.
- **`data()`**: 문자열 데이터에 대한 포인터를 반환합니다.
