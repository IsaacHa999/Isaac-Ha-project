...
public class IntQueue implements Cloneable
{
   ...
   **/
   public void ensureCapacity(int minimumCapacity) /// 좀 복잡하다.
   {
      int biggerArray[ ];
      int n1, n2;
   ...
   *   arithmetic overflow.
   **/    
   public void insert(int item) ///Enqueue
   {
      ...
      if (manyItems == 0)   ///constructior에서 f r 정의 x
      {
         front = 0;
         rear = 0;
      }
      else
         rear = nextIndex(rear);    ///rear에 item을 삽입!!!
   ...
   **/
   ...
   {
      if (++i == data.length) /// 맨 끝이면 처음으로 간다. == (i+1) % data.length
         return 0;
      else
         return i;
   }
   ...
   **/   
   public void trimToSize( )    ///불필요한 공간 삭제
   {
      int trimmedArray[ ];
      int n1, n2;
      ...
      {  // Create trimmed array and copy data[front]...data[rear] into it.
         ...
         System.arraycopy(data, front, trimmedArray, front [x -->0]??, manyItems);
         data = trimmedArray;
      }
      else
